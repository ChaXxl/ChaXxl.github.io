...
1. 下载 [清华大学开源软件镜像站](https://mirrors.tuna.tsinghua.edu.cn/anaconda/miniconda/)
...
1. [Node.js 下载](https://www.nodejs.com.cn/download.html)

    ~~~shell
    sudo wget https://npmmirror.com/mirrors/node/v18.19.0/node-v18.19.0-linux-arm64.tar.xz
    ~~~

2. 解压

    ~~~shell
    sudo tar -xJf node-v18.19.0-linux-arm64.tar.xz
    ~~~
...
1. 创建数据库
    ...
    ~~~mysql
    create database xxldb;
    ~~~

2. 创建用户

    ~~~mysql
    create user 'xxl'@'%' identified with mysql_native_password by '4444';
    ~~~

3. 赋予权限

    ~~~mysql
    GRANT ALL PRIVILEGES ON xxldb.* from 'xxl'@'%';
    flush privileges;
    ~~~
...
1.  [官网下载](https://www.postgresql.org/download/)
2. 配置文件在 `/etc/postgresql/16/main/postgresql.conf`



#### 启动远程访问PostgreSQL服务器数据库
...
2. 重启使生效
    ...
    ~~~shell
    sudo systemctl restart postgresql 或
    sudo service postgresql restart
    ~~~

3. 编辑 `pg_hba.conf` 文件,配置服务器接受远程连接
